...
class Net(Model):
	...
	def __init__(self, config, input_shape=None):
		...
		self.fc_mu5 = nn.Linear(self.OUTPUT_FMAP_SIZE[self.BN5], self.NUM_LATENT_VARS)  # bn5_output_size-dimensional x, NUM_LATENT_VARS-dimensional output
		self.fc_var5 = nn.Linear(self.OUTPUT_FMAP_SIZE[self.BN5], self.NUM_LATENT_VARS)  # bn5_output_size-dimensional x, NUM_LATENT_VARS-dimensional output
		...
		self.fc_mu6 = nn.Linear(self.OUTPUT_FMAP_SIZE[self.BN6], self.NUM_LATENT_VARS)  # bn6_output_size-dimensional x, NUM_LATENT_VARS-dimensional output
		self.fc_var6 = nn.Linear(self.OUTPUT_FMAP_SIZE[self.BN6], self.NUM_LATENT_VARS)  # bn6_output_size-dimensional x, NUM_LATENT_VARS-dimensional output
		...
		self.fc_mu7 = nn.Linear(self.OUTPUT_FMAP_SIZE[self.BN7], self.NUM_LATENT_VARS)  # bn7_output_size-dimensional x, NUM_LATENT_VARS-dimensional output
		self.fc_var7 = nn.Linear(self.OUTPUT_FMAP_SIZE[self.BN7], self.NUM_LATENT_VARS)  # bn7_output_size-dimensional x, NUM_LATENT_VARS-dimensional output
		...
		self.fc_mu8 = nn.Linear(self.OUTPUT_FMAP_SIZE[self.BN8], self.NUM_LATENT_VARS)  # bn8_output_size-dimensional x, NUM_LATENT_VARS-dimensional output
		self.fc_var8 = nn.Linear(self.OUTPUT_FMAP_SIZE[self.BN8], self.NUM_LATENT_VARS)  # bn8_output_size-dimensional x, NUM_LATENT_VARS-dimensional output
		...
		self.fc_mu9 = nn.Linear(4096, self.NUM_LATENT_VARS)  # 4096-dimensional x, NUM_LATENT_VARS-dimensional output
		self.fc_var9 = nn.Linear(4096, self.NUM_LATENT_VARS)  # 4096-dimensional x, NUM_LATENT_VARS-dimensional output
		...
		self.dec_fc2 = nn.Linear(self.NUM_LATENT_VARS, self.OUTPUT_FMAP_SIZE[self.BN8])  # NUM_LATENT_VARS-dimensional x, bn8_output_size-dimensional output
		...
		self.dec_fc3 = nn.Linear(self.NUM_LATENT_VARS, self.OUTPUT_FMAP_SIZE[self.BN7])  # NUM_LATENT_VARS-dimensional x, bn7_output_size-dimensional output
		...
		self.dec_fc4 = nn.Linear(self.NUM_LATENT_VARS, self.OUTPUT_FMAP_SIZE[self.BN6])  # NUM_LATENT_VARS-dimensional x, bn6_output_size-dimensional output
		...
		self.dec_fc5 = nn.Linear(self.NUM_LATENT_VARS, self.OUTPUT_FMAP_SIZE[self.BN5])  # NUM_LATENT_VARS-dimensional x, bn5_output_size-dimensional output
		...
		self.dec_fc6 = nn.Linear(self.NUM_LATENT_VARS, self.OUTPUT_FMAP_SIZE[self.BN4])  # NUM_LATENT_VARS-dimensional x, bn4_output_size-dimensional output
		...
		self.dec_fc7 = nn.Linear(self.NUM_LATENT_VARS, self.OUTPUT_FMAP_SIZE[self.BN3])  # NUM_LATENT_VARS-dimensional x, bn3_output_size-dimensional output
		...
		self.dec_fc8 = nn.Linear(self.NUM_LATENT_VARS, self.OUTPUT_FMAP_SIZE[self.BN2])  # NUM_LATENT_VARS-dimensional x, bn2_output_size-dimensional output
		...
		self.dec_fc9 = nn.Linear(self.NUM_LATENT_VARS, self.OUTPUT_FMAP_SIZE[self.BN1])  # NUM_LATENT_VARS-dimensional x, bn1_output_size-dimensional output
	...
	def local_updates(self):
		if self.ALPHA != 0: # Local updates are enabled
			self.fc9.weight.grad = self.ALPHA * self.fc9_delta_w + (1 - self.ALPHA) * self.fc9.weight.grad
			self.fc9.weight.bias = self.ALPHA * self.fc9_delta_bias + (1 - self.ALPHA) * self.fc9.bias.grad
			self.bn9.weight.grad = self.ALPHA * self.bn9_delta_w + (1 - self.ALPHA) * self.bn9.weight.grad
			self.bn9.weight.bias = self.ALPHA * self.bn9_delta_bias + (1 - self.ALPHA) * self.bn9.bias.grad
			self.fc_mu9.weight.grad = self.ALPHA * self.fc_mu9_delta_w + (1 - self.ALPHA) * self.fc_mu9.weight.grad
			self.fc_mu9.weight.bias = self.ALPHA * self.fc_mu9_delta_bias + (1 - self.ALPHA) * self.fc_mu9.bias.grad
			self.fc_var9.weight.grad = self.ALPHA * self.fc_var9_delta_w + (1 - self.ALPHA) * self.fc_var9.weight.grad
			self.fc_var9.weight.bias = self.ALPHA * self.fc_var9_delta_bias + (1 - self.ALPHA) * self.fc_var9.bias.grad
			...
			self.conv8.weight.grad = self.ALPHA * self.conv8_delta_w + (1 - self.ALPHA) * self.conv8.weight.grad
			self.conv8.weight.bias = self.ALPHA * self.conv8_delta_bias + (1 - self.ALPHA) * self.conv8.bias.grad
			self.bn8.weight.grad = self.ALPHA * self.bn8_delta_w + (1 - self.ALPHA) * self.bn8.weight.grad
			self.bn8.weight.bias = self.ALPHA * self.bn8_delta_bias + (1 - self.ALPHA) * self.bn8.bias.grad
			self.fc_mu8.weight.grad = self.ALPHA * self.fc_mu8_delta_w + (1 - self.ALPHA) * self.fc_mu8.weight.grad
			self.fc_mu8.weight.bias = self.ALPHA * self.fc_mu8_delta_bias + (1 - self.ALPHA) * self.fc_mu8.bias.grad
			self.fc_var8.weight.grad = self.ALPHA * self.fc_var8_delta_w + (1 - self.ALPHA) * self.fc_var8.weight.grad
			self.fc_var8.weight.bias = self.ALPHA * self.fc_var8_delta_bias + (1 - self.ALPHA) * self.fc_var8.bias.grad
			...
			self.conv7.weight.grad = self.ALPHA * self.conv7_delta_w + (1 - self.ALPHA) * self.conv7.weight.grad
			self.conv7.weight.bias = self.ALPHA * self.conv7_delta_bias + (1 - self.ALPHA) * self.conv7.bias.grad
			self.bn7.weight.grad = self.ALPHA * self.bn7_delta_w + (1 - self.ALPHA) * self.bn7.weight.grad
			self.bn7.weight.bias = self.ALPHA * self.bn7_delta_bias + (1 - self.ALPHA) * self.bn7.bias.grad
			self.fc_mu7.weight.grad = self.ALPHA * self.fc_mu7_delta_w + (1 - self.ALPHA) * self.fc_mu7.weight.grad
			self.fc_mu7.weight.bias = self.ALPHA * self.fc_mu7_delta_bias + (1 - self.ALPHA) * self.fc_mu7.bias.grad
			self.fc_var7.weight.grad = self.ALPHA * self.fc_var7_delta_w + (1 - self.ALPHA) * self.fc_var7.weight.grad
			self.fc_var7.weight.bias = self.ALPHA * self.fc_var7_delta_bias + (1 - self.ALPHA) * self.fc_var7.bias.grad
			...
			self.conv6.weight.grad = self.ALPHA * self.conv6_delta_w + (1 - self.ALPHA) * self.conv6.weight.grad
			self.conv6.weight.bias = self.ALPHA * self.conv6_delta_bias + (1 - self.ALPHA) * self.conv6.bias.grad
			self.bn6.weight.grad = self.ALPHA * self.bn6_delta_w + (1 - self.ALPHA) * self.bn6.weight.grad
			self.bn6.weight.bias = self.ALPHA * self.bn6_delta_bias + (1 - self.ALPHA) * self.bn6.bias.grad
			self.fc_mu6.weight.grad = self.ALPHA * self.fc_mu6_delta_w + (1 - self.ALPHA) * self.fc_mu6.weight.grad
			self.fc_mu6.weight.bias = self.ALPHA * self.fc_mu6_delta_bias + (1 - self.ALPHA) * self.fc_mu6.bias.grad
			self.fc_var6.weight.grad = self.ALPHA * self.fc_var6_delta_w + (1 - self.ALPHA) * self.fc_var6.weight.grad
			self.fc_var6.weight.bias = self.ALPHA * self.fc_var6_delta_bias + (1 - self.ALPHA) * self.fc_var6.bias.grad
			...
			self.conv5.weight.grad = self.ALPHA * self.conv5_delta_w + (1 - self.ALPHA) * self.conv5.weight.grad
			self.conv5.weight.bias = self.ALPHA * self.conv5_delta_bias + (1 - self.ALPHA) * self.conv5.bias.grad
			...
			self.dec_fc6.weight.grad = self.ALPHA * self.dec_fc6_delta_w + (1 - self.ALPHA) * self.dec_fc6.weight.grad
			self.dec_fc6.weight.bias = self.ALPHA * self.dec_fc6_delta_bias + (1 - self.ALPHA) * self.dec_fc6.bias.grad
			self.dec_bn6.weight.grad = self.ALPHA * self.dec_bn6_delta_w + (1 - self.ALPHA) * self.dec_bn6.weight.grad
			self.dec_bn6.weight.bias = self.ALPHA * self.dec_bn6_delta_bias + (1 - self.ALPHA) * self.dec_bn6.bias.grad
			self.dec_conv6.weight.grad = self.ALPHA * self.dec_conv6_delta_w + (1 - self.ALPHA) * self.dec_conv6.weight.grad
			self.dec_conv6.weight.bias = self.ALPHA * self.dec_conv6_delta_bias + (1 - self.ALPHA) * self.dec_conv6.bias.grad
			...
			self.dec_fc7.weight.grad = self.ALPHA * self.dec_fc7_delta_w + (1 - self.ALPHA) * self.dec_fc7.weight.grad
			self.dec_fc7.weight.bias = self.ALPHA * self.dec_fc7_delta_bias + (1 - self.ALPHA) * self.dec_fc7.bias.grad
			self.dec_bn7.weight.grad = self.ALPHA * self.dec_bn7_delta_w + (1 - self.ALPHA) * self.dec_bn7.weight.grad
			self.dec_bn7.weight.bias = self.ALPHA * self.dec_bn7_delta_bias + (1 - self.ALPHA) * self.dec_bn7.bias.grad
			self.dec_conv7.weight.grad = self.ALPHA * self.dec_conv7_delta_w + (1 - self.ALPHA) * self.dec_conv7.weight.grad
			self.dec_conv7.weight.bias = self.ALPHA * self.dec_conv7_delta_bias + (1 - self.ALPHA) * self.dec_conv7.bias.grad
			...
			self.dec_fc8.weight.grad = self.ALPHA * self.dec_fc8_delta_w + (1 - self.ALPHA) * self.dec_fc8.weight.grad
			self.dec_fc8.weight.bias = self.ALPHA * self.dec_fc8_delta_bias + (1 - self.ALPHA) * self.dec_fc8.bias.grad
			self.dec_bn8.weight.grad = self.ALPHA * self.dec_bn8_delta_w + (1 - self.ALPHA) * self.dec_bn8.weight.grad
			self.dec_bn8.weight.bias = self.ALPHA * self.dec_bn8_delta_bias + (1 - self.ALPHA) * self.dec_bn8.bias.grad
			self.dec_conv8.weight.grad = self.ALPHA * self.dec_conv8_delta_w + (1 - self.ALPHA) * self.dec_conv8.weight.grad
			self.dec_conv8.weight.bias = self.ALPHA * self.dec_conv8_delta_bias + (1 - self.ALPHA) * self.dec_conv8.bias.grad
			...
			self.dec_fc9.weight.grad = self.ALPHA * self.dec_fc9_delta_w + (1 - self.ALPHA) * self.dec_fc9.weight.grad
			self.dec_fc9.weight.bias = self.ALPHA * self.dec_fc9_delta_bias + (1 - self.ALPHA) * self.dec_fc9.bias.grad
			self.dec_bn9.weight.grad = self.ALPHA * self.dec_bn9_delta_w + (1 - self.ALPHA) * self.dec_bn9.weight.grad
			self.dec_bn9.weight.bias = self.ALPHA * self.dec_bn9_delta_bias + (1 - self.ALPHA) * self.dec_bn9.bias.grad
			self.dec_conv9.weight.grad = self.ALPHA * self.dec_conv9_delta_w + (1 - self.ALPHA) * self.dec_conv9.weight.grad
			self.dec_conv9.weight.bias = self.ALPHA * self.dec_conv9_delta_bias + (1 - self.ALPHA) * self.dec_conv9.bias.grad