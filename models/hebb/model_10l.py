...
class Net(Model):
	...
	def __init__(self, config, input_shape=None):
		...
		self.conv1 = H.HebbianConv2d(
			...
			reduction=self.RED,
			alpha=self.ALPHA,
		) # 3 x channels, 8x12=96 output channels, 7x7 convolutions
		...
		self.conv2 = H.HebbianConv2d(
			...
			lrn_sim=self.LRN_SIM,
			lrn_act=F.relu,
			lrn_cmp=H.Competitive(out_size=(8, 16), competitive_act=self.COMPETITIVE_ACT, k=self.K),
			out_sim=self.OUT_SIM,
			out_act=F.relu,
			out_cmp=None,
			gating=self.GATING,
			...
			reduction=self.RED,
			alpha=self.ALPHA,
		)  # 96 x channels, 8x16=128 output channels, 3x3 convolutions
		...
		self.conv4 = H.HebbianConv2d(
			...
			reduction=self.RED,
			alpha=self.ALPHA,
		)  # 192 x channels, 12x16=192 output channels, 3x3 convolutions
