...
class Model(nn.Module):
	...
	def __init__(self, input_shape=None, num_classes=10, num_hidden=4096, dropout_p=0., hebb_param_dict=None):
		...
		# First convolutional layer: 3 input channels, 96 output channels, 7x7 convolutions
		self.conv1 = self.hfactory.create_hebb_layer(in_channels=3, out_channels=96, kernel_size=7, teacher_distrib=0)
		self.bn1 = nn.BatchNorm2d(96)  # Batch Norm layer
		
		# Second convolutional layer: 96 input channels, 128 output channels, 3x3 convolutions
		...
		# Third convolutional layer: 128 input channels, 192 output channels, 3x3 convolutions
		...
		# Fourth convolutional layer: 192 input channels, 192 output channels, 3x3 convolutions
		self.conv4 = self.hfactory.create_hebb_layer(in_channels=192, out_channels=192, kernel_size=3, teacher_distrib=160//self.NUM_CLASSES)
		self.bn4 = nn.BatchNorm2d(192)  # Batch Norm layer
		
		# Fifth convolutional layer: 192 input channels, 256 output channels, 3x3 convolutions
		self.conv5 = self.hfactory.create_hebb_layer(in_channels=192, out_channels=256, kernel_size=3, teacher_distrib=240//self.NUM_CLASSES)
		self.bn5 = nn.BatchNorm2d(256)  # Batch Norm layer
		
		# Sixth convolutional layer: 256 input channels, 256 output channels, 3x3 convolutions
		self.conv6 = self.hfactory.create_hebb_layer(in_channels=256, out_channels=256, kernel_size=3, teacher_distrib=240//self.NUM_CLASSES)
