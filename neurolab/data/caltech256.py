...
class Caltech256(ImageFolder):
	def __init__(self, root, download=False, **kwargs):
		...
		self.CALTECH256_ZIP_PATH = os.path.join(self.root, self.CALTECH256_ZIP_FILE)
		self.CALTECH256_FOLDER = os.path.join(self.root, '256_ObjectCategories')
		...
		if not os.path.exists(self.CALTECH256_FOLDER):
			...
			if not os.path.exists(self.CALTECH256_ZIP_PATH):
				...
				print("Downloading {} file...".format(self.CALTECH256_ZIP_FILE))
				download_large_file_from_drive(self.CALTECH256_DRIVE_ID, self.CALTECH256_ZIP_PATH)
				print("Done!")
...
# Data manager class for Caltech256
class Caltech256DataManager(DataManager):
	def __init__(self, config):
		self.CALTECH256_SIZE = 30607
		self.indices = list(range(self.CALTECH256_SIZE))
		super().__init__(config)
	
	def get_dataset_metadata(self):
		return {
			P.KEY_DATASET: 'caltech256',
			P.KEY_DS_TRN_SET_SIZE: 25607,
			P.KEY_DS_VAL_SET_SIZE: 5000,
			P.KEY_DS_TST_SET_SIZE: 5000,
			P.KEY_DS_INPUT_SHAPE: (3, 224, 224),
			P.KEY_DS_NUM_CLASSES: 257
		}
	...
	def get_sampler(self, split):
		# split is a Subset of Caltech256. split.dataset is Caltech256. From this we can access the number of samples
	...
	def get_train_split(self, transform=None, num_samples=None):
		...
		return Subset(Caltech256(root=self.DATASET_FOLDER, download=True, transform=transform), self.indices[:num_samples])
	...
	def get_val_split(self, transform=None, num_samples=None):
		...
		if self.TRN_SET_SIZE - self.TOT_TRN_SAMPLES >= num_samples:
			...
			return Subset(Caltech256(root=self.DATASET_FOLDER, download=True, transform=transform), self.indices[self.TRN_SET_SIZE - num_samples:self.TRN_SET_SIZE])
		# Validate directly on test set otherwise
		return Subset(Caltech256(root=self.DATASET_FOLDER, download=True, transform=transform), self.indices[self.TRN_SET_SIZE:self.TRN_SET_SIZE + num_samples])
